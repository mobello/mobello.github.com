...
/**
 * Renders SplitViewController
 * @class
 * @private
 */
tau.ui.SplitViewController.prototype.renderer = tau.mixin({
  $base: 'tau-splitView',
  $styleClass: {
    splitviewContainer: 'tau-spContainer',
    leftContainer: 'tau-spLeftContainer',
    leftNaviBar: 'tau-spLeftNaviBar',
    separateBar: 'tau-separateBar',
    rightContainer: 'tau-spRightContainer',
    rightNaviBar: 'tau-spRightNaviBar',
    popOver : 'tau-popOverContainer'
  },

  _template:['<div class="${base} ${splitviewContainer}">',
             '<div class=${leftContainer}>',
             '<div class=${leftNaviBar}></div>',
             '</div>',
             '<div class=${separateBar}></div>',
             '<div class=${rightContainer}>',
             '<div class=${rightNaviBar}></div>',
             '</div>',             
             '</div>'],
             
  _initializeDOM: function ($dom, $base, root) {
    $dom[tau.ui.SplitViewController.LEFT_KEY] = root.childNodes[0];
    $dom[tau.ui.SplitViewController.LEFT_NAVIBAR_KEY] = root.childNodes[0].childNodes[0];
    $dom[tau.ui.SplitViewController.SEPARATEBAR_KEY] = root.childNodes[1];    
    $dom[tau.ui.SplitViewController.RIGHT_KEY] = root.childNodes[2];
    $dom[tau.ui.SplitViewController.RIGHT_NAVIBAR_KEY] = root.childNodes[2].childNodes[0];
  },
  
  getParentElement: function ($renderData, key, createElement) {
    var $dom = $renderData.$dom;
    switch (key) {
    case tau.ui.SplitViewController.LEFT_KEY:
      return $dom[key];
    case tau.ui.SplitViewController.SEPARATEBAR_KEY:
      return $dom[key];
    case tau.ui.SplitViewController.RIGHT_KEY:
      return $dom[key];
    case tau.ui.SplitViewController.LEFT_NAVIBAR_KEY:
      return $dom[key];
    case tau.ui.SplitViewController.RIGHT_NAVIBAR_KEY:
      return $dom[key];
    default:
      return tau.ui.BaseRenderer.getParentElement.apply(this, arguments);
    }
  ...
  setLeftCtrlToPopOver: function ($renderData, opts) {
    var $dom = $renderData.$dom;
    $dom[tau.ui.SplitViewController.LEFT_KEY].style.display = 'none';
    $dom[tau.ui.SplitViewController.LEFT_KEY].style.position = 'absolute'; 
    $dom[tau.ui.SplitViewController.LEFT_KEY].style.height = opts.popoverHeightSize;
    $dom[tau.ui.SplitViewController.SEPARATEBAR_KEY].style.width = '0px';
    
    tau.util.dom.addClass($dom[tau.ui.SplitViewController.LEFT_KEY],
        this.$styleClass.popOver); 
  },
  
  setLeftCtrlToLeftContainer : function ($renderData) {
    var $dom = $renderData.$dom;
    $dom[tau.ui.SplitViewController.LEFT_KEY].style.display = '';
    $dom[tau.ui.SplitViewController.LEFT_KEY].style.position = 'relative'; 
    $dom[tau.ui.SplitViewController.LEFT_KEY].style.height = '';
    $dom[tau.ui.SplitViewController.SEPARATEBAR_KEY].style.width = '2px';
    tau.util.dom.removeClass($dom[tau.ui.SplitViewController.LEFT_KEY],
        this.$styleClass.popOver);
  },
  
  setOption: function ($renderData, opts) {
    var $dom = $renderData.$dom;
    $dom[tau.ui.SplitViewController.LEFT_KEY].style.width = opts.leftControllerWidth;
  }
