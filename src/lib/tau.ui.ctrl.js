...
//------------------------------------------------------------------------------
/** @lends tau.ui.SplitViewController */
$class('tau.ui.SplitViewController').extend(tau.ui.Controller).define({
  $static: /** @lends tau.ui.SplitViewController */{
    /** $dom에서 왼쪽 컨테이너 해당하는 key값 */
    LEFT_KEY : 'leftContainer',
    /** $dom에서 왼쪽 NavigationBar key값 */
    LEFT_NAVIBAR_KEY : 'leftNaviBar',
    /** $dom에서 가운데 분리 영역 key값 */
    SEPARATEBAR_KEY : 'separateBar',
    /** $dom에서 오른쪽 컨테이너 key값 */
    RIGHT_KEY : 'rightContainer',
    /** $dom에서 오른쪽 NavigatonBar key값 */
    RIGHT_NAVIBAR_KEY : 'rightNaviBar',
    /** setController에서 사용하는 Left Index값 */
    LEFT_INDEX : 0,
    /** setController에서 사용하는 Right Index값 */
    RIGHT_INDEX : 1
  },
  
  /**
   * 생성자, 새로운 SplitViewController객체를 생성한다.
   * @class 2개의 하위 controller를 포함하여 관리한다. SplitViewController는 
   * 최상위 controller 이여야 한다. landscape와 portrait를 모두 지원한다. 
   * portrait에서는 popover 버튼이 나타나서 왼쪽 컨트롤러를 나타나게 한다.
   * @example
   * <p>
   * var splitViewCtrl = new tau.ui.SplitViewController({
   *      popoverHeightSize : "328px",
   *      leftControllerWidth : "328px",
   ...
   * @extends tau.ui.Controller
   * @constructs
   * @param {Object} [opts] SplitViewController 옵션
   */
  SplitViewController: function (opts) {
    var that = this;
    /** @private SplitViewController에서 관리하는 controllers array */
    this.$controllers = [];  
    this.$renderData = this.renderer.initialize();
    /** @private SplitViewController에서 관리하는 popover 버튼 지시자 (사용금지)*/
    this.$popover = null;
    /** @private SplitViewController에서 popover maskingFn (사용금지)*/
    this.$popoverMaskingFn = null;
    /** @private SplitViewController에서 popover Text (사용금지)*/
    this.$popoverText = null;
    
    
    this.opts = tau.mixin({
      popoverHeightSize: '600px',
      leftControllerWidth : '300px',
      orientationSupport : true,
      hideHeader : true
    }, opts, true);
    
    this.renderer.setOption(this.$renderData, this.opts);
    
    //orientation에 대한 이벤트 핸들러.
    if(this.opts.orientationSupport){
      tau.getRuntime().onEvent(tau.rt.Event.ORIENTATION,
          function (e, orientationAngle) {
            that._checkOrientation(e, orientationAngle);
            if(this.$popoverMaskingFn !== null) {
              this._getRightActiveCtrl().getScene()
              .unsubscribeEvent(tau.rt.Event.TAP, this.$popoverMaskingFn, this);
              this.$popoverMaskingFn = null;
            }
      },this);
    }
  ...
  doDraw: function (parent, callbackFn) {
    tau.util.dom.pushElement(parent, this.getDOM());
    tau.ui.SplitViewController.$super.doDraw.apply(this, arguments);
  },
  
  /**
   * SplitViewController가 차지하는 영역에서 주어진 파라미터(key)에 따라 해당되는 DOM객체를
   * 반환한다. 만약 key가 지정되지 않으면 현재 SplitViewController의 최상위 DOM앨리먼트를
   * 찾아 반환한다. 만약 SplitViewController의 Content영역에 해당하는 DOM앨리먼트를
   * 반환하고자 한다면 'tau.ui.CONTENT_KEY'를 사용하면 된다.
   * @param {String} key SplitViewController의 각 영영에 해당되는 키 값
   * @returns {HTMLElement} DOM element
   */
  getDOM: function (key) {
    return this.renderer.getParentElement(this.$renderData, key);
  ...
  /**
   * Popover Button를 반환한다. landscape에서는 null를 반환한다.
   * 이 객체은 계속 유지되지 않는다.
   * @returns {tau.ui.Button}
   */
  getPopoverButton: function () {
    return this.$popover;
  },
  
  /**
   * popover 버튼에 text를 설정한다.
   * @param text
   */
  
  setPopoverText: function (text) {
    this.$popoverText = text;
  },
  
  /**
   * popover 버튼에 text를 반환한다.
   * @returns
   */
  getPopoverText: function () {
    return this.$popoverText;
  },
  
  /**
   * SplitViewController 에서 leftController 와 rightController를 설정한다.
   * @param leftController 왼쪽 컨테이너에 설정할 controller
   * @param rightController 오른쪽 컨테이너에 설정할 controller
   * @returns {Boolean} 설정이 되었다면 true, 설정이 안 되었다면 false  
   */  
  SetControllers: function (leftController, rightController) { // setControllers 로 변경 필요
    if(leftController && rightController 
        && !(leftController instanceof tau.ui.Controller) 
        && !(rightController instanceof tau.ui.Controller)) {
      tau.log.error("controller is not suitable");
      return false;
    }
    this.$popover = null;
    
    if(this.$controllers[0]) this._destroyController(this.$controllers[0]);
    if(this.$controllers[1]) this._destroyController(this.$controllers[1]);
    
    this.$controllers[0] = leftController;
    this.$controllers[1] = rightController;
    
    this.$controllers[0].$parent = this;
    this.$controllers[0].setBubble(this);
    
    this.$controllers[1].$parent = this;
    this.$controllers[1].setBubble(this);
      
    this._controllerTransition(tau.ui.SplitViewController.LEFT_KEY,this.$controllers[0]);
    this._controllerTransition(tau.ui.SplitViewController.RIGHT_KEY,this.$controllers[1]);
    
    if(this.opts.orientationSupport){
      this._setEventSequenceNavi(this.$controllers[1]);
    }
    
    if(!this.opts.hideHeader){
      this._setNaviBar(0, this.$controllers[0]);
      this._setNaviBar(1, this.$controllers[1]);
    }
  ...
  getSubDelegators: function () {
    ...
    if(this.$popover !== null){
      var array = [this.$controllers[0],this.$controllers[1],this.$popover];
      return array;
    }else {
  ...
  /**
   * SplitViewController에 index에 맞게 controller 설정한다 
   * Left Index값 LEFT_INDEX, Right Index값 RIGHT_INDEX
   * controller설정하면 이전 controller는 삭제 된다.   *    
   * @param index left 또는 right 컨테이너에 구분은 하는 index (LEFT_INDEX, RIGHT_INDEX) 
   * @param controller 설정하는 controller
   * @returns {Boolean} 설정이 되었다면 true, 설정이 안 되었다면 false
   */
  setController: function (index, controller) {    
    if(index < 0 || index > 1){
      tau.log.error("index should be 0 or 1 ");
      return false;
    } 
    //this.$eventMgr.setEnable(false);
    this.$popover = null;
    
    this._removeNaviBar(this.$controllers[index]);
    
    var key = (index==0?tau.ui.SplitViewController.LEFT_KEY
        :tau.ui.SplitViewController.RIGHT_KEY);
        
    controller.$parent = this;
    controller.setBubble(this);
    this.$controllers[index].getDOM().style.display = 'none';
    this._destroyController(this.$controllers[index]);
    this.$controllers[index] = controller;    
    
    this._controllerTransition(key,controller);
    
    if(!this.opts.hideHeader){
      this._setNaviBar(index, controller);
    }
    ...
    if(this.opts.orientationSupport){
      ...
      if(this.isProtraitType()){
        this._createPopoverButton();
        this.$renderData.$dom[tau.ui.SplitViewController.LEFT_KEY].style.display = 'none';
      }
    }
  ...
  /**
   * index에 맞게 controller를 반환한다.
   * @param index left 또는 right 컨테이너에 구분은 하는 index (LEFT_INDEX, RIGHT_INDEX)
   * @returns {Object} controller 또는 없으면 null
   */
  ...
  /**
   * 현재 portrait 면 true를 반환한다.
   * @returns {Boolean} portrait면 true, 그렇지 않는면 false
   */
  isProtraitType: function (){
  ...
  /**
   * portrait mode로 변경되었을 때의 callback 함수
   * 변경이 완료 되어진 후 호출된다.
   */
  didPortraitMode: function () {
    
  },
  
  /**
   * landscape mode로 변경되었을 때의 callback 함수
   * 변경이 완료 되어진 후 호출된다.
   */
  didLandscapeMode: function () {
    
  },
  
  /**
   * controller를 새로 생성하고 새로 그린다.
   * 이전 controller에서 새로운 controller로 변경할 때 호출한다.
   * @param key 구분하기위한 key(tau.ui.SplitViewController.LEFT_KEY,
   *  tau.ui.SplitViewController.RIGHT_KEY)   
   * @param controller
   */
  _controllerTransition: function (key, controller) {
    var that = this;
    controller.doInit(function () {
      controller.doDraw(
          that.renderer.getParentElement(that.$renderData,key, true),
          function() {
          });
    });     
  },
  ...
  /**
   * controller를 삭제한다.
   * @param controller 삭제하는 controller
   */
  _destroyController: function (controller) {
  ...
  /**
   * 현재의 orientation를 검사하여 landscape, portrait에 맞는 형태로 변경한다.
   * @param e 이벤트 null도 허용
   * @param orientationAngle 현재의 angle값
   */
  _checkOrientation: function (e, orientationAngle) {
    if(orientationAngle){
      if(orientationAngle === 0 || orientationAngle === 180){
        this._changeLeftCtrlToPopover();
      }else{
        this._changeLeftCtrlToLeftContainer();
      }
    }else {
      if(tau.getWidth() <= 768){
        this._changeLeftCtrlToPopover(); 
  ...
  /**
   * leftController를 leftContainer에서 위치에서 popover버튼의 형태로 변경한다.
   * 그리고leftContainer를 숨긴다.
   */
  _changeLeftCtrlToPopover: function () {
    this.renderer.setLeftCtrlToPopOver(this.$renderData,this.opts);    
    this._createPopoverButton();
    this.didPortraitMode();
    
  },
  
  /**
   * leftController를 popover버튼 형태에서 leftContainer으로 변경한다.
   * popover버튼은 삭제한다.
   */
  _changeLeftCtrlToLeftContainer: function () {
    this.renderer.setLeftCtrlToLeftContainer(this.$renderData);    
    this._removePopoverButton();
    this.didLandscapeMode();
  },
  
  /**
   * popover 버튼을 생성한다. 이전에 leftNaviItem이 있으면 패널에 넣은 후 
   * popover버튼을 넣는다.
   */
  _createPopoverButton: function () {
    if(this.$popover === null){
      var that = this;
      var popover = new tau.ui.Button();      
      var popoverToggleFn = function(e, payload) {
        var $dom = that.$renderData.$dom;
        var display  = $dom[tau.ui.SplitViewController.LEFT_KEY].style.display;
        var ctrl = that._getRightActiveCtrl();
        if(display && display === 'none'){          
          that._addMaskingPopover(ctrl);
          $dom[tau.ui.SplitViewController.LEFT_KEY].style.display = '';
        }else{
          that._removeMaskingPopover(ctrl);
          $dom[tau.ui.SplitViewController.LEFT_KEY].style.display = 'none';
        }
      };
      popover.publishEvent(tau.rt.Event.TAP, {
        defaultFn : popoverToggleFn
      });      
      popover.setLabel(this.$popoverText===null?'library':this.$popoverText);
      
      this.$popover = popover;
      var navBar = this._getRightActiveNaviBar();     
      var leftItem = navBar.getMapItem(tau.ui.NavigationBar.LEFT_KEY);
      if(leftItem){
        var panel = new tau.ui.Panel();
        panel.setStyles({'-webkit-box-orient': 'horizontal',
          'display': '-webkit-box'});
        navBar.removeLeftItem();
        panel.add(leftItem);
        panel.add(popover);
        leftItem = panel;
      }else {
      ...
      navBar.setLeftItem(leftItem);      
      navBar.draw(this.$renderData.$dom[tau.ui.SplitViewController.RIGHT_NAVIBAR_KEY]);
      navBar._centerTitle();
    }
  },
  
  /**
   * popover버튼을 삭제한다. 이전에 panel 형태로 들어가 있다면 panel에서 첫번째 item만
   * 빼서 다시 넣는다.
   * @returns
   */
  _removePopoverButton: function () {
    if(this.$popover !== null){
      var navBar = this._getRightActiveNaviBar();     
      var leftItem = navBar.getMapItem(tau.ui.NavigationBar.LEFT_KEY);
      if(leftItem){
        if(leftItem instanceof tau.ui.Panel){
          //복합 left item,  panel과 popover 을 지워서 새로 삽입 
          leftItem.remove(this.$popover);
          var item = leftItem.getComponents();
          if(item.length  != 1){
            return Error("The size of NaviBar's LeftItem isn't 1, It's size must be 1");
          }
          navBar.removeLeftItem();
          navBar.setLeftItem(item[0]);          
  ...
  /**
   * rightController의 activeBar를 반환한다.
   * @returns
   */
  _getRightActiveNaviBar: function () {
   // if(this.$controllers[1] instanceof tau.ui.SequenceNavigator){ //FIXME
    if(this.$controllers[1] instanceof tau.ui.NavigationController){
      return this.$controllers[1].getActiveNavigationBar();
    }else if(this.$controllers[1] instanceof tau.ui.SceneController){
      return this._createNaviBar(this.$controllers[1]); 
    }else {
      throw new Error("Can't support Controller");
    }
  },
  
  /**
   * rightController의 activeController를 반환한다.
   * @returns
   */
  _getRightActiveCtrl: function () {
    // if(this.$controllers[1] instanceof tau.ui.SequenceNavigator){ //FIXME
     if(this.$controllers[1] instanceof tau.ui.NavigationController){
       return this.$controllers[1].getActiveController();
     }else if(this.$controllers[1] instanceof tau.ui.SceneController){
  ...
  /**
   * naviBar를 생성한다.(이전에 존재하면 css를 변경한다.)
   * @param controller
   * @returns
   */
  _createNaviBar: function (controller) {
      var navi = controller.getNavigationBar();      
      tau.util.dom.addClass(navi.getDOM(), 'tau-navbar');
      navi.setStyles({'background-color' : 'black' ,'position' : 'relative' });
      return navi;
  },
   ...
   * @param controller
   */
  _removeNaviBar: function (controller) {
    if(controller instanceof tau.ui.SceneController){
      if(controller.$navBar){
        controller.$navBar.destroy();
      }
    }
  },
  
  /**
   * naviBar를 없으면 만들어서 새로 그린다.
   * @param index key 구분하기위한 key(tau.ui.SplitViewController.LEFT_KEY,
   *  tau.ui.SplitViewController.RIGHT_KEY)  
   * @param controller naviBar를 그릴 controller
   */
  _setNaviBar: function (index, controller) {
    if(controller instanceof tau.ui.SceneController){
      var navi = this._createNaviBar(controller);
      if(index == 0){
        navi.draw(this.$renderData.$dom[tau.ui.SplitViewController.LEFT_NAVIBAR_KEY]);
      }else{
        navi.draw(this.$renderData.$dom[tau.ui.SplitViewController.RIGHT_NAVIBAR_KEY]);
      }
    }
  },
  
  /**
   * sequenceNavi에 push pop 할 때에 이벤트처리를 위한 리슨너 추가한다.
   * @param controller
   */
  _setEventSequenceNavi: function (controller) {
    if(controller instanceof tau.ui.SequenceNavigator){
      var that = this;
      controller.onEvent(tau.ui.SequenceNavigator.EVENT_PUSH_FINISH,
           function (e) {
             that._checkOrientation(e, window.orientation);
           }
      );
      controller.onEvent(tau.ui.SequenceNavigator.EVENT_PUSH_START,
          function (e) {
            that._removePopoverButton();
          }
      );
      controller.onEvent(tau.ui.SequenceNavigator.EVENT_POP_FINISH,
          function (e) {
            that.$popover = null;
            that._checkOrientation(e, window.orientation);
          }
  ...
  /**
   * popover버튼으로 leftController가 나타났을 경우 masking 이벤트를 추가한다.
   * @param controller
   */
  _addMaskingPopover: function (controller) {
    this.$popoverMaskingFn = 
    function (e) {            
      this.$renderData.$dom[tau.ui.SplitViewController.LEFT_KEY].style.display = 'none';
      e.stopPropagation();
      controller.getScene().unsubscribeEvent(tau.rt.Event.TAP, this.$popoverMaskingFn, this);
      this.$popoverMaskingFn = null;
    };
    controller.getScene().onEvent(tau.rt.Event.TAP, this.$popoverMaskingFn ,this, true);
  },
  
  /**
   * popover버튼으로 leftController가 사라질 때 masking 이벤트를 삭제한다.
   * @param controller
   */
  _removeMaskingPopover: function (controller) {
    controller.getScene().unsubscribeEvent(tau.rt.Event.TAP, this.$popoverMaskingFn, this);
    this.$popoverMaskingFn = null;
  }
});
